...
"""
This script collects more accurate runtime of GATR General models (Collect runtime of every single data instance). Always put the spec of problem 5 as args, then based on the actual problem type we want to solve, we change mtsp_problem_type, this ensure we load the right data file.

For help:
python eval_gat_generalize_runtime.py --help

Usage:
python eval_gat_generalize_runtime.py
# method param
...
Example Command:
python eval_gat_generalize_runtime.py --method_mtsp_problem_type 5 --method_seed 0 --seed 10 --mtsp_problem_type 5 --num_node 24 --num_task 6 --discretize_level 4 --num_agent 6 --batch_size 1000
"""
...
def main():
    ...
    mission_times = []
    actions = []
    small_env = get_env(5)(method_generator)

    for bs in tqdm(
        range(args.batch_size), desc=f"Running {args.batch_size} samples one by one"
    ):
        # setup small td_init
        small_td = small_env.generator(1).to(device)
